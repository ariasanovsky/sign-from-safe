use candle_core::{safetensors::{load_buffer, Load}, Device, Tensor};
...
fn load_consistent_with_candle() {
    let a = Tensor::arange(0f32, 6f32, &Device::Cpu).unwrap().reshape((2, 3)).unwrap().to_dtype(candle_core::DType::BF16).unwrap();
